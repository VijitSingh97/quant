...
| `btcvol.carryscan` / `btcvol-carry-scan` | **Cross-asset carry scanner.** Ranks every Hyperliquid perp by funding APR (the carry yield), filtered to liquid markets, so capital goes where the carry is richest. Flags spot-leg availability and microcap-funding traps. `--min-oi`, `--top`. |
